...
void MKZ_DRAW_init(){
	...
	modelView_st = 0;
	float * matrix = MKZ_ARITHMETIC_create_matrix();
	MKZ_ARITHMETIC_identityMatrix(matrix);
	MKZ_DATASTRUCT_stack_push(&modelView_st,matrix,MKZ_GENERIC_ID_MATRIX);
...
void MKZ_DRAW_set_cameraMat(float * camMat){
	MKZ_ARITHMETIC_to_cameraMatrix(camMat,baseChange_mat);
}
...
void MKZ_DRAW_object(MKZ_mesh * mo){
	...
	MKZ_DATASTRUCT_stack_peek(modelView_st,&matrix_v,&type_id);
	matrix = (float*) matrix_v;
	MKZ_geometry * mesh = mo->mesh;
	...
	for (v = 0 ; v < mesh->num_vertices ; v++){
		...
		temp[0] = mesh->vertex_table[v].coord.x;
		temp[1] = mesh->vertex_table[v].coord.y;
		temp[2] = mesh->vertex_table[v].coord.z;

		MKZ_ARITHMETIC_transform(matrix,temp);
...
void MKZ_DRAW_add_light(MKZ_light * lo){
	...
	if (lo->light_type != MKZ_LIGHT_TYPE_DIRECTIONAL)
		glLightfv(light_ind, GL_POSITION, matrix+12);
	else{
	...
	if (lo->light_type == MKZ_LIGHT_TYPE_FOCAL){
		...
		float det = MKZ_ARITHMETIC_determinant(matrix);
		//printf("det: %f\n",det);
...
void MKZ_DRAW_push_modelview(float * f){

	MKZ_DATASTRUCT_stack_push(&modelView_st,f,MKZ_GENERIC_ID_MATRIX);


}


void MKZ_DRAW_pop_modelview(){
	void * temp;
	int t_i;
	MKZ_DATASTRUCT_stack_pop(&modelView_st,&temp,&t_i);
}
